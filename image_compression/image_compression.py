...
def pca_compression(matrix: NDArray, p: int) -> Tuple[NDArray, NDArray, NDArray]:
    ...
    row_means = np.mean(matrix, axis=1)[:, None]
    row_centered_matrix = matrix / row_means

    # Найдем матрицу ковариации
    cov_matrix = np.cov(row_centered_matrix)
    ...
    eigh_vectors_num = eigh_vectors.shape[1]

    # Сортируем собственные значения в порядке убывания
    sorted_eigh_values_idxs = np.flip(np.argsort(eigh_values))
    sorted_eigh_values = eigh_values[sorted_eigh_values_idxs]
    ...
    sorted_eigh_vectors = eigh_vectors[:, sorted_eigh_values_idxs]

    # Оставляем только p собственных векторов
    top_p_eigh_vectors = sorted_eigh_vectors[:, :p]

    # Проекция данных на новое пространство
    data_projection = np.dot(top_p_eigh_vectors.T, row_centered_matrix)
    
    return top_p_eigh_vectors, data_projection, row_means
