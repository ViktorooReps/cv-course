...
def run_single_test(data_dir, output_dir):
    ...
    assert n_rows > min_n_rows and n_cols > min_n_cols, f'aligned image is too small. Given: {n_rows}x{n_cols} with minimum of {int(min_n_rows)}x{int(min_n_cols)} (original shape: {img.shape[:2]})'
...
def check_test(data_dir):
    ...
    if diff > diff_max:
        res = f'Wrong answer blue: {b_row}, {b_col} (correct: {gt_b_row}, {gt_b_col}), red: {r_row}, {r_col} (correct: {gt_r_row}, {gt_r_col})'
    else:
        res = f'Ok'
